...
pub struct MultiCircuitSolidityGenerate<'a, C: CurveAffine, const N: usize> {
    // verifier circuit just means the TestCircuit: no public inputs. we want to generate a proof of this circuit and then genreate solidity code to verify
    pub verify_params: &'a Params<C>,
    pub verify_vk: &'a VerifyingKey<C>,
    // serialized instance
    // pub verify_circuit_instance: Vec<Vec<Vec<C::ScalarExt>>>,
    // serialized final proof generated by TestCircuit (?)
    pub proof: Vec<u8>,
...
impl<'a, C: CurveAffine, const N: usize> MultiCircuitSolidityGenerate<'a, C, N> {
    pub fn call<E: MultiMillerLoop<G1Affine = C, Scalar = C::ScalarExt>>(
        ...
        // we have no target circuit so ignore
        /*
        ...
        */
        ...
        let verify_params = self
            .verify_params
            .verifier::<E>(0 /*self.verify_public_inputs_size*/)
            .unwrap();
        ...
        // this is converting the proof transcript into a format using solidity chips instead of halo2 chips
        let mut transcript =
        ...
        // no public inputs so instances all empty
        ...
        */
        ...
        println!("Finished CodegenTranscriptRead.\nctx.enter_instance");
        ctx.enter_instance();
        let (_, assigned_instances) =
            assign_instance_commitment(ctx, schip, pchip, &[], &self.verify_vk, &verify_params)
                .unwrap();
        ctx.exit_instance();
        println!("ctx.exit_instance done");
        ...
        .unwrap();
        println!("done with verify single proof  no eval");
